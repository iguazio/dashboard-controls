 ...
 */
function build(next) {
    gulp.series(lint, clean, injectVersion, vendorLess, vendorJs, gulp.parallel(appLess, appJs, fonts, images, i18n), function (next) {
        gulp.src(config.build_dir + '/**/*')
            .pipe(gulp.dest('../nuclio/pkg/dashboard/ui/node_modules/iguazio.dashboard-controls/dist'))
            .pipe(gulp.dest('../dashboard/node_modules/iguazio.dashboard-controls/dist'));
        next();
    })(next);
}
