...
(function () {
  ...
  angular
    ...
    .component('nclFunctionFromTemplate', {
      ...
      templateUrl:
        'nuclio/common/screens/create-function/function-from-template/function-from-template.tpl.html',
      controller: FunctionFromTemplateController,
    });

  function FunctionFromTemplateController(
    $element,
    $window,
    $scope,
    $state,
    ...
    function createFunction() {
      ...
      if (
        ctrl.functionFromTemplateForm.$valid &&
        !lodash.isNil(ctrl.selectedTemplate)
      ) {
        ...
        if (
          lodash.isEmpty(ctrl.project) &&
          ctrl.selectedProject.id !== 'new_project'
        ) {
          ctrl.project = lodash.find(ctrl.projects, [
            'metadata.name',
            ctrl.selectedProject.id,
          ]);
        }
        ...
        if (lodash.has(ctrl.functionData, 'template')) {
          ngDialog
            .open({
              template:
                '<ncl-function-from-template-dialog data-close-dialog="closeThisDialog(template)" data-template="$ctrl.functionData"></ncl-function-from-template-dialog>',
              plain: true,
              scope: $scope,
              className:
                'ngdialog-theme-nuclio function-from-template-dialog-wrapper',
            })
            .closePromise.then(function (data) {
              if (!lodash.isNil(data.value)) {
                ...
                ctrl
                  .renderTemplate({
                    template: lodash.omit(ctrl.functionData, [
                      'rendered',
                      'metadata',
                      'ui',
                    ]),
    ...
    function isCreateFunctionAllowed() {
      return lodash.isEmpty(
        lodash.get(ctrl, 'functionFromTemplateForm.$error'),
      );
    }
    ...
    function selectTemplate(templateName) {
      if (!lodash.isEqual(templateName, ctrl.selectedTemplate)) {
        ...
        // assign new template
        ctrl.functionData = angular.copy(
          ctrl.templatesWorkingCopy[ctrl.selectedTemplate],
        );
      }
    }
    ...
    function filterByRuntime(template) {
      return (
        ctrl.selectedRuntimeFilter.id === 'all' ||
    ...
    function filterByTitleAndDescription(template) {
      ...
      return (
        lodash.isEmpty(ctrl.searchQuery) ||
        lodash.includes(title, ctrl.searchQuery) ||
        lodash.includes(description, ctrl.searchQuery)
      );
    }
    ...
    function goToEditCodeScreen() {
      ...
      ctrl
        .getFunction({ metadata: { name: ctrl.functionName } })
        .then(function (existingFunction) {
          ctrl.toggleSplashScreen({ value: false });
          FunctionsService.openFunctionConflictDialog(
            ctrl.project,
    ...
    function initFunctionData() {
      ...
      ctrl
        ...
        .catch(function (error) {
          var defaultMsg = $i18next.t(
            'functions:ERROR_MSG.GET_FUNCTIONS_TEMPLATE',
            { lng: lng },
          );
    ...
    function getRuntimeFilters() {
      return [
        ...
        },
        {
          ...
          name:
            'Python 3.7 ' +
            $i18next.t('functions:DEPRECATED_SOON_LABEL', { lng: lng }),
          nameTemplate:
            'Python 3.7 ' +
            '<b>' +
            $i18next.t('functions:DEPRECATED_SOON_LABEL', { lng: lng }) +
        ...
        },
        {
          ...
          name:
            '.NET Core ' +
            $i18next.t('functions:TECH_PREVIEW_LABEL', { lng: lng }),
          visible: true,
        },
        {
          id: 'java',
          name:
            'Java ' + $i18next.t('functions:TECH_PREVIEW_LABEL', { lng: lng }),
          visible: true,
        },
        {
          id: 'nodejs',
        ...
        },
        {
          id: 'shell',
          name:
            'Shell ' + $i18next.t('functions:TECH_PREVIEW_LABEL', { lng: lng }),
          visible: true,
        },
      ];
    }
    ...
    function paginateTemplates() {
      ...
      var pageSize =
        $window.innerWidth > 1453 && $window.innerWidth < 1822 ? 9 : 8;

      ctrl.templatesWorkingCopy = lodash
        .chain(templatesOriginalObject)
        .filter(filterByRuntime)
        .filter(filterByTitleAndDescription)
        .thru(function (filteredTemplates) {
          ctrl.page.total = Math.ceil(
            lodash.size(filteredTemplates) / pageSize,
          );

          return lodash.slice(
            filteredTemplates,
            ctrl.page.number * pageSize,
            ctrl.page.number * pageSize + pageSize,
          );
        })
        .keyBy(function (template) {
          return (
            template.rendered.metadata.name.split(':')[0] +
            ' (' +
            template.rendered.spec.runtime +
            ')'
          );
        })
        .value();
    ...
    function prepareProjects() {
      ...
      ctrl.selectedProject = lodash.isNil(ctrl.selectedProject)
        ? newProject
        : ctrl.selectedProject;

      ctrl.projectsList = lodash
        .chain(ctrl.projects)
      ...
      ctrl.selectedProject = lodash.isEmpty(ctrl.projectsList)
        ? newProject
        : ctrl.selectedProject.id !== 'new_project'
        ? ctrl.selectedProject
    ...
    function setLastLineClass() {
      ...
      var elementsPerLine = Math.floor(
        parseInt(templatesWrapper.css('width')) / TEMPLATE_WIDTH,
      );
      var countLastLineElements =
        lodash.size(templates) % elementsPerLine || elementsPerLine;
    ...
    function setReadMoreButtonsState(templates) {
      ...
      lodash.forEach(templates, function (template) {
        ...
        var templateElement = lodash.find(templatesElements, [
          'innerHTML',
          description,
        ]);
        ...
        lodash.set(
          ...
          templateElement.scrollHeight >
            angular.element(templateElement).height(),
        );
      });
    }
  }
